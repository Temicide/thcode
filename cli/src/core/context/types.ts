...
export function effectiveContextCapacity(raw: number, output = 0): number { return validCapacity(raw, output) ? Math.max(0, raw - Math.max(output, Math.ceil(raw * 0.08)) - Math.max(2048, Math.ceil(raw * 0.02))) : 0 }
